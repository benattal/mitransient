...
def parse_args():
    ...
    parser.add_argument('--peak-width', type=int, default=10,
                        help='Half-width of direct peak mask in bins (default: 10). Bins within [peak-width, peak+width] are masked as direct.')
    ...
    parser.add_argument('--mixed-pixel-threshold', type=float, default=None,
                        help='Filter out mixed pixels where indirect energy exceeds this fraction of total energy (e.g., 0.1 = 10%%). These pixels likely see multiple surfaces. Default: None (no filtering)')
...
def detect_direct_peaks(transient, start_opl, bin_width, threshold_percentile=95, peak_width=10,
...
def visualize_depths(relay_depths, transient, direct_mask, start_opl, bin_width,
                     output_dir, output_name, use_log_scale=True, pixel_range=None):
    """
...
def main():
    ...
    print(f"\n[3/6] Detecting direct peaks (peak_width={args.peak_width} bins)...")
    relay_depths, direct_mask, indirect, valid_mask = detect_direct_peaks(
        transient_direct, start_opl, bin_width, peak_width=args.peak_width
    )
    ...
    # Filter out mixed pixels (pixels with high indirect energy ratio)
    if args.mixed_pixel_threshold is not None:
        print(f"\n  Filtering mixed pixels (indirect/total > {args.mixed_pixel_threshold:.1%})...")

        # Compute per-pixel total and indirect energy
        total_energy_per_pixel = np.sum(transient_direct, axis=-1)  # (H, W)
        indirect_energy_per_pixel = np.sum(indirect, axis=-1)  # (H, W)

        # Compute indirect energy ratio (avoid division by zero)
        with np.errstate(divide='ignore', invalid='ignore'):
            indirect_ratio = np.where(
                total_energy_per_pixel > 0,
                indirect_energy_per_pixel / total_energy_per_pixel,
                0.0
            )

        # Mark pixels with high indirect ratio as invalid (mixed pixels)
        mixed_pixel_mask = indirect_ratio > args.mixed_pixel_threshold
        n_mixed_before = mixed_pixel_mask.sum()
        n_valid_before = valid_mask.sum()

        # Update valid_mask to exclude mixed pixels
        valid_mask = valid_mask & ~mixed_pixel_mask

        n_valid_after = valid_mask.sum()
        n_filtered = n_valid_before - n_valid_after

        print(f"    Mixed pixels detected: {n_mixed_before} ({100*n_mixed_before/mixed_pixel_mask.size:0.0001f}%)")
        print(f"    Valid pixels before: {n_valid_before}, after: {n_valid_after}")
        print(f"    Pixels filtered out: {n_filtered}")
        print(f"    Indirect ratio range: [{indirect_ratio[valid_mask].min():.3f}, {indirect_ratio[valid_mask].max():.3f}] (remaining valid)")

    # Determine output name
    output_name = args.output_name or os.path.dirname(args.transient_file) + '_direct_bp'
    ...
    # Visualize depths (on full data, with red outline if pixel range specified)
    ...
    # Zero out indirect data for invalid pixels (mixed pixels, no signal, etc.)
    # This ensures they don't contribute to backprojection
    n_invalid = (~valid_mask).sum()
    if n_invalid > 0:
        print(f"\n  Zeroing out {n_invalid} invalid pixels for backprojection...")
        indirect[~valid_mask] = 0
        indirect_rgb[~valid_mask] = 0
