"""
Render transient scene using the time-gated transient path integrator.

This script renders a scene using the TimeGatedTransientPath integrator,
which samples target times and evaluates emitter pulses at (target_time - path_length).
"""
...
# Import the time-gated integrator to register the plugin
from mitransient.integrators.timegated_transientpath import TimeGatedTransientPath

# Parse command-line arguments
parser = argparse.ArgumentParser(description='Render transient scene with time-gated integrator')
...
# Time-gated specific arguments
parser.add_argument('--pulse-type', type=str, default='gaussian',
                    choices=['gaussian', 'histogram'],
                    help='Pulse shape type (default: gaussian)')
parser.add_argument('--pulse-width', type=float, default=0.01,
                    help='Gaussian pulse width in OPL (default: 0.01)')
parser.add_argument('--pulse-file', type=str, default=None,
                    help='Path to histogram pulse .npy file')
parser.add_argument('--sensor-response', type=str, default='box',
                    choices=['box', 'gaussian'],
                    help='Sensor response type (default: box)')
parser.add_argument('--sensor-response-width', type=float, default=None,
                    help='Sensor response width in OPL')
...
# Load scene and override integrator
scene_dict = mi.xml.load_dict(scene_path)

# Build integrator properties
integrator_props = {
    'type': 'timegated_transient_path',
    'max_depth': 8,
    'pulse_type': args.pulse_type,
    'pulse_width_opl': args.pulse_width,
    'sensor_response_type': args.sensor_response,
}

if args.sensor_response_width is not None:
    integrator_props['sensor_response_width_opl'] = args.sensor_response_width

if args.pulse_type == 'histogram' and args.pulse_file is not None:
    integrator_props['pulse_values'] = args.pulse_file

# Override the integrator
scene_dict['integrator'] = integrator_props
...
print(f"Pulse type: {args.pulse_type}")
print(f"Pulse width: {args.pulse_width} OPL")
...
if args.output_name:
    ...
    output_name = os.path.splitext(os.path.basename(scene_path))[0] + '_timegated'
...
if not args.no_plot:
    ...
    ax.set_title(f'Time-Gated Transient at pixel ({pixel_x}, {pixel_y})\nPulse: {args.pulse_type} (width={args.pulse_width})')
    ax.legend()
