...
def create_cam_version(input_file, output_file):
    ...
    for child in integrator.findall("boolean"):
        if child.get("name") == "wall_sample":
            print(f"Found parameter 'wall_sample'. Changing value from '{child.get('value')}' to 'false'.")
            child.set("value", "false")
            param_found = True
            break
    
    if not param_found:
        print("Warning: Could not find <boolean name='wall_sample'> inside the integrator.")
...
if __name__ == "__main__":
    ...
    create_cam_version(args.input_file, args.output_file)