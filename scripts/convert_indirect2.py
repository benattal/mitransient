...
def create_nlos_only_version(input_file, output_file):
    ...
    for child in integrator.findall("boolean"):
        if child.get("name") == "filter_direct":
            print(f"Found parameter 'filter_direct'. Changing value from '{child.get('value')}' to 'true'.")
            child.set("value", "true")
            param_found = True
            break
    
    if not param_found:
        print("Warning: Could not find <boolean name='filter_direct'> inside the integrator.")
        # Optional: You could create it if it's missing
        ET.SubElement(integrator, "boolean", name="filter_direct", value="true")
        print(f"added filter_direct = True")
