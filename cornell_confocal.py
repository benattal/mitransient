...
projector = mi.load_dict({
    ...
    "fov": 0.55,
    "is_confocal": True,  # Confocal mode - frame computed dynamically from camera ray
...
output_video_path = os.path.join(os.path.dirname(__file__), 'results', 'confocal_output.mp4')
mitr.vis.save_video(
    output_video_path,
...
output_npy_path = os.path.join(os.path.dirname(__file__), 'results', 'confocal_transient.npy')
