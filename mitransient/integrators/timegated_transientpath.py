...
class TimeGatedTransientPath(ADIntegrator):
    ...
    def __init__(self, props: mi.Properties):
        ...
        # ADIntegrator already parsed these and normalized max_depth=-1 to
        # Mitsuba's unsigned infinite-depth sentinel. Do not overwrite them
        # with the raw signed property value.
    ...
    def sample(self,
        ...
        while dr.hint(active, max_iterations=self.max_depth,
            ...
            # BSDF evaluation
            wo = si.to_local(ds.d)
            bsdf_value, _ = bsdf.eval_pdf(bsdf_ctx, si, wo, active_em)
            bsdf_value = si.to_world_mueller(bsdf_value, -wo, si.wi)

            # This integrator only accumulates the projector-sampled estimator:
            # paths generated by the BSDF sampler never score a projector hit.
            # Applying an MIS weight without that competing estimator biases the
            # result low.
            Lr_dir = β * bsdf_value * em_weight * pulse_weight
    ...
    def _render_chunk(self,
        ...
        if self.time_sampling == "stratified":
            ...
            # Randomize the cyclic offset so every temporal bin has the
            # correct marginal probability even when spp < num_strata.
            # Without this, low-spp renders sample only the first spp bins
            # while still applying a full-range importance weight.
            stratum_offset = mi.UInt32(sampler.next_1d() * num_strata)
            stratum_idx = (sample_within_pixel + stratum_offset) % num_strata
